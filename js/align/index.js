...
const SETTING_ENABLED = "MKTheme.Align.Enabled";
const SETTING_SNAP_DIST = "MKTheme.Align.SnapDistance";
...
const ICON_URL = new URL("../assets/icons/ui/align-center-v.svg", import.meta.url).href;
...
function injectToolbarCSS() {
  if (document.getElementById("mk-theme-align-css")) return;
  const style = document.createElement("style");
  style.id = "mk-theme-align-css";
  style.textContent = `
    .mk-theme-align-btn .mk-theme-align-icon {
    ...
    .mk-theme-align-btn:not(.mk-theme-align-on) {
      background-color: #2a2c2e !important;
      color: #ddd !important;
      border-color: #444 !important;
    }
    .mk-theme-align-btn:not(.mk-theme-align-on):hover {
      background-color: #3a3d40 !important;
    }
    .mk-theme-align-btn.mk-theme-align-on {
      background-color: ${BRAND} !important;
      color: #fff !important;
      border-color: ${BRAND} !important;
    }
    .mk-theme-align-btn.mk-theme-align-on:hover {
      background-color: ${BRAND} !important;
...
function updateToolbarTint() {
  ...
  if (!btn) return;
  btn.classList.toggle("mk-theme-align-on", state.enabled);
}
...
function mountToolbarButton() {
  ...
  if (!settingsGroupEl) {
    ...
    if (++mountToolbarButton._tries > 20) {
      console.warn("[MKTheme.Align] 工具栏挂载失败：未找到 app.menu.settingsGroup");
      return;
    }
  ...
  btn.className = "comfyui-button mk-theme-align-btn";
  btn.title = "开关 MK 主题吸附与对齐参考线（拖拽时按住 Shift 可临时跳过吸附）";
  // Keep `.comfyui-button` defaults for OFF state (matches other unfilled
  // buttons like the bookmark icon). The `.mk-theme-align-on` class swaps in
  // BRAND background + white icon when active, mirroring the Manager button.
  btn.innerHTML = `<span class="mk-theme-align-icon"></span>`;
  ...
  group.className = "comfyui-button-group mk-theme-align-group";
...
app.registerExtension({
  ...
  settings: [
    {
      ...
      defaultValue: false,
      category: ["MK 主题", "对齐"],
      tooltip: "拖拽节点或分组时，自动吸附到其他节点/分组的边缘与中心线；调整节点大小时也会吸附。按住 Shift 可临时跳过吸附（Alt 已被 ComfyUI 用作拖拽复制）。",
      onChange: (v) => {
        state.enabled = !!v;
        updateToolbarTint();
      },
    },
    {
      ...
      category: ["MK 主题", "对齐（高级）"],
      tooltip: "边缘距离参考线多近时触发吸附，单位为屏幕像素。",
      onChange: (v) => {
        const n = Number(v);
  ...
  setup() {
    ...
    try { window.MKThemeAlign = { snapMovingRect, snapResizeCorner, endExternalDrag: clearExternalGuides }; } catch (_e) {}
  },
});
...
function installDrawHook() {
  ...
  if (typeof proto?.drawFrontCanvas !== "function") {
    console.warn("[MKTheme.Align] 未找到 LGraphCanvas.drawFrontCanvas，对齐参考线将无法绘制");
    return;
  }
